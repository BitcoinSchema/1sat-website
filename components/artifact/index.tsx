...
const Artifact: React.FC<ArtifactProps> = ({
  ...
  const content = useMemo(() => {
    ...
    return type === ArtifactType.Video ? (
      ...
      <div
        className={`relative flex items-center justify-center w-full h-full rounded ${showZoom ? "h-auto" : "h-full"
          }`}
      >
        {src !== "" && src !== undefined && (
          <ImageWithFallback
  ...
  const wrapperClassName = `${showFooter
      ...
      : ""
    } ${glow ? "glow" : ""} flex flex-col items-center justify-center bg-card w-full h-full relative rounded ${to && !disableLink ? "cursor-pointer" : "cursor-default"
  ...
  const footerContent = showFooter === true && num !== undefined && (
    <div className="text-xs absolute bottom-0 left-0 bg-black/75 flex items-center justify-between w-full p-2 h-[56px]">
      ...
      <Button
        variant="secondary"
        size="sm"
        className={`h-auto py-1 px-2 text-[#aaa] bg-[#222] hover:bg-emerald-600 hover:text-white transition ${price !== undefined ? "cursor-pointer" : ""
          }`}
        onClick={(e) => {
