      ...
      {mode === "prove" ? (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {proveInput.map((word, index) => (
              <div key={`prove-slot-${index}`} className="relative">
                <span className="absolute left-2 top-2.5 text-xs text-muted-foreground select-none">
                  {index + 1}.
                </span>
                <div className={`pl-8 font-mono h-10 flex items-center rounded-md border ${word ? "bg-muted" : "bg-background border-dashed"
                  }`}>
                  {word || <span className="text-muted-foreground">Select word</span>}
                </div>
              </div>
            ))}
          </div>
            ...
            {shuffledWords.map((word, i) => {
              ...
              return (
                <Button
                  ...
                  onClick={() => {
                    const firstEmpty = proveInput.findIndex((w) => w === "");
                    if (firstEmpty !== -1) {
                      setProveInput((prev) => {
                        const next = [...prev];
          ...
          {inputMnemonic.map((word, index) => (
            <div key={`word-input-${index}-${word}`} className="relative">
              ...
              <Input
                ...
                readOnly={mode === "view"}
                className={`pl-8 font-mono ${!bip39words.includes(word) && word && mode === "edit"
