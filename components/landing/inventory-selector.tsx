...
import { Ordinal } from "@/lib/wallet/gorillapool-service";
...
export function InventorySelector({ open, onOpenChange, onSelect }: InventorySelectorProps) {
  ...
  const filteredTokens = bsv21Tokens.filter(t => {
    if (!search) return true;
    const name = t.data?.bsv20?.tick || t.data?.bsv20?.sym || t.txid.slice(0, 8);
    return name.toLowerCase().includes(search.toLowerCase());
  });
      ...
      <DialogContent className="max-w-3xl h-[600px] flex flex-col p-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden">
          <Tabs defaultValue="ordinals" className="h-full flex flex-col">
            ...
            <TabsContent value="ordinals" className="flex-1 overflow-y-auto p-6">
              {!isInitialized && (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredOrdinals.map((item) => {
                  ...
                  const name = num ? `Inscription #${num}` : `Ordinal ${item.txid.slice(0, 4)}...${item.txid.slice(-4)}`;
                  ...
                  return (
                    <div
                      key={outpoint}
                      className="border rounded-lg overflow-hidden hover:border-primary cursor-pointer transition-colors bg-card"
                      onClick={() => {
                        onSelect({
                          id: outpoint,
                          name,
            ...
            <TabsContent value="tokens" className="flex-1 overflow-y-auto p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredTokens.map((item) => {
                  ...
                  const tick = item.data?.bsv20?.tick || item.data?.bsv20?.sym || "TOKEN";
                  ...
                  return (
                    ...
                    >
                      <div className="aspect-square relative bg-muted/20 flex flex-col items-center justify-center p-4">
                        ...
                        <Badge variant="outline" className="mt-2 text-[10px] break-all max-w-full">
                          {tick}
                        </Badge>
                      </div>
                      <div className="p-3">
                        <div className="font-medium text-sm truncate">{amt} {tick}</div>
                        <div className="text-xs text-muted-foreground mt-1 truncate">
                          BSV-20
                        </div>
                      </div>
