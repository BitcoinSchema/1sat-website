...
import { Center, Environment, Float, Torus } from "@react-three/drei";
...
function Scene() {
  ...
  return (
    <>
      ...
      <spotLight position={[-10, 0, 10]} intensity={2} color={controls.innerColor} />

      <Environment preset="city" />

      <EffectComposer>
        <Bloom luminanceThreshold={1.5} intensity={1.0} radius={0.5} mipmapBlur />
      </EffectComposer>
  ...
  useFrame((state) => {
    if (outerRef.current && controls.speed > 0) {
      ...
      if (controls.gyroMode) {
        outerRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.5) * 0.2;
        outerRef.current.rotation.z = Math.cos(state.clock.elapsedTime * 0.3) * 0.1;
      }
    }
  });
