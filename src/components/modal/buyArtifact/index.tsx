...
const BuyArtifactModal: React.FC<BuyArtifactModalProps> = ({
	...
	const knownScammer = useMemo(() => {
		return (
			listing.owner &&
			listing.owner &&
			SCAM_LISTING_USER_BLACKLIST.some((u) => u.address === listing.owner)
		);
	}, [listing.owner]);
			...
			<DialogContent className="bg-background border-border rounded-lg max-w-lg max-h-[90vh] overflow-y-auto">
				...
				{scamListing && (
					<div className="p-3 border border-red-500/50 bg-red-900/20 text-red-400">
						...
						<p className="font-mono text-xs text-red-300">
							This item has been identified as potentially inauthentic, or a
							duplicate of an original work and is not available for purchase.
						</p>
					</div>
				)}
				...
				{knownScammer && (
					<div className="p-3 border border-red-500/50 bg-red-900/20 text-red-400">
						...
						<p className="font-mono text-xs text-red-300">
							This seller has been identified as a known scammer and their
							listings are not available for purchase.
						</p>
					</div>
				)}
				...
				{!scamListing && showLicense && (
					<div className="p-3 border border-border bg-card">
						...
						<p className="font-mono text-xs text-muted-foreground">
							...
							and control of the associated token. This purchase does not
							include a license to any artwork or IP that may be depicted here
