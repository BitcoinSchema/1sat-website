...
const WalletMenu: React.FC = () => {
	...
	const showManageProfileModal = useSignal(false);
	...
	const handleManageProfile = () => {
		showManageProfileModal.value = true;
		showDropdown.value = false;
	};
  ...
  const identityImage = idImage && getImageFromGP(idImage);
	...
	return (
		<ul className="dropdown dropdown-end">
			...
			>
				<div
					className={`tooltip tooltip-bottom ${
						identityImage
							? "rounded-full border-2 w-9 h-9 relative overflow-hidden"
							: ""
					}`}
				...
				>
					{identityImage ? (
						...
						<MdAccountCircle
							size={"1.3rem"}
							color={identityColor ? `rgb(${identityColor})` : ""}
						/>
					)}
				</div>
			</div>
			...
			/>
			<ManageProfileModal
				open={showManageProfileModal.value}
				onClose={() => {
					showManageProfileModal.value = false;
				}}
			/>
