...
import type { CollectionItemSubTypeData, CollectionItemTrait, RarityLabels, CollectionTraits } from 'js-1sat-ord';
    ...
    <div className="mt-4">
      ...
      <div className="mb-2">
        ...
        <input
          ...
          value={mintNumber || ''}
          onChange={(e) => setMintNumber(e.target.value ? Number(e.target.value) : undefined)}
        />
      </div>
      ...
      {traits && Object.entries(traits).map(([traitName, traitData]) => (
        <div key={traitName} className="mb-2">
          <label className="block">{traitName}</label>
          <select
            className="select select-bordered w-full"
            value={selectedTraits.find(t => t.name === traitName)?.value || ''}
