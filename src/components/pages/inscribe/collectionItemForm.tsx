...
const CollectionItemForm: React.FC<CollectionFormProps> = ({
  ...
  const selectedCollectionData = userCollections?.find(
		(c) => c.outpoint === selectedCollection,
	);
  const collectionSubTypeData = selectedCollectionData?.data?.map?.subTypeData
	...
	return (
		<div className="my-4">
				...
				<label className="label cursor-pointer">
					<div className="label-text flex items-center">
						{collectionEnabled ? (
							...
							<div>
								<span className="text-yellow-200/50 mr-2">
									Single Inscription
								</span>
						...
						<div
							className="tooltip"
							data-tip="A collection must be created before adding collection items."
						>
			...
			{collectionEnabled && (
				<>
					<label
						...
						className="font-medium flex justify-between mb-2"
					>
          ...
          setSubTypeData={setSubTypeData}
					rarityLabels={collectionSubTypeData?.rarityLabels}
					traits={collectionSubTypeData?.traits}
				/>
			)}
		</div>
	);
