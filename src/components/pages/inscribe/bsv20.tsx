...
import "buffer";
import type { Payment, Utxo } from "js-1sat-ord";
...
const InscribeBsv20: React.FC<InscribeBsv20Props> = ({ inscribedCallback }) => {
	...
	const changeDecimals = useCallback(
		(e: any) => {
			setDecimals(
				e.target.valuye ? Number.parseInt(e.target.value, 10) : undefined,
			);
		},
		[setDecimals],
	);
		...
		(e: any) => {
			if (selectedActionType === ActionType.Mint && selectedBsv20?.lim) {
				if (
					...
					Number.parseInt(selectedBsv20.lim, 10)
				) {
	...
	const submitDisabled = useMemo(() => {
		return (
			...
			(!!limit &&
				!!maxSupply &&
				Number.parseInt(maxSupply, 10) < Number.parseInt(limit, 10))
		);
	}, [
		inscribeStatus,
		tickerAvailable,
