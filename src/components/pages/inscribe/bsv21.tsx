...
const InscribeBsv21: React.FC<InscribeBsv21Props> = ({ inscribedCallback }) => {
	...
	const onCropComplete = useCallback(
		(_croppedArea: CropData, croppedAreaPixels: CroppedArea) => {
	...
	const handleCropIcon = useCallback(async () => {
		if (!selectedImage || !croppedAreaPixels || !selectedImageURL) {
			console.error(
				"No image selected or no crop area",
				selectedImage,
				croppedAreaPixels,
			);
		...
		try {
			const dataUrl = await getCroppedImg(selectedImageURL, croppedAreaPixels);
			if (dataUrl) {
			...
			>
				...
				>
					...
					<DialogFooter className="justify-end mt-4">
						<Button type="button" onClick={handleCropIcon} className="mr-2">
							Crop
						</Button>
						<Button
							type="button"
