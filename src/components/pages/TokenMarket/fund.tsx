				...
				{!ticker.included && indexingFeeUsd.value.length > 0 && (
					<>
						...
						<div>
							...
							<div
								className="tooltip"
								data-tip="Payment required per action"
							>
								<FaQuestionCircle className="inline" />
							</div>
						</div>
						...
						{isFinite(Number(usdReceived.value)) ? (
							isFinite(Number(indexingFeeUsd.value)) && (
								<div className="text-right">
									{`$${usdReceived.value} / $${indexingFeeUsd.value}`}
								</div>
							)
						) : (
