...
import { type AssetType, toastErrorProps } from "@/constants";
...
export function TokenMarketTabs({ ticker, show, type }: Props) {
	useSignals();
	...
	const hasBalance = computed(() => {
		return (
			ticker &&
			bsv20Balances.value?.some((b) => {
				return (
					(b.tick === ticker.sym && b.all.confirmed - b.listed.confirmed > 0) ||
					(b.tick === ticker.tick && b.all.confirmed - b.listed.confirmed > 0)
				);
			})
		);
	});
		...
		ticker && (
						...
						<div className="relative w-1/2">
							<div className="flex border-b border-border mr-4 px-4">
								<button
									type="button"
									className={clsx(
										"px-4 py-3 font-mono text-xs uppercase tracking-wider transition border-b-2 -mb-[1px]",
										{
							...
							{hasBalance.value && (
								<button
									...
									className="absolute right-4 top-2 h-9 w-9 flex items-center justify-center rounded bg-yellow-900/30 text-yellow-400 border border-yellow-500/50 hover:bg-yellow-900/50 transition"
								>
          ...
          <div className="flex flex-wrap items-center border-b border-border p-2 gap-1.5">
							<button
								type="button"
								className={clsx(
									"px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-wider transition",
									{
				...
				<CreateTokenListingModal
					...
					open={
						showAddListingModal.value === ticker.id ||
