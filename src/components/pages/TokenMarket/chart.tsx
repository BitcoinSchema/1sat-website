...
const TremorChartComponent: FC<ChartProps> = ({
	...
	const dataset = computed(() => {
		const data =
			dataCategory === DataCategory.Listings ? listings.value : sales.value;
		if (!data) {
			return [];
		}
		// Extract raw data and sort by block height
		const rawData = [...data].sort((a, b) => {
			if (a.height === 0) {
				-1;
			}
			if (b.height === 0) {
		...
		const groupedData = rawData.reduce(
			(acc, item) => {
				...
				const amt = ticker.dec
					? Number.parseFloat(item.amt) / 10 ** ticker.dec
					: Number.parseFloat(item.amt);
