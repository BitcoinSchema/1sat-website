...
export const CollectionList = ({initialMarketItems, initialCollectionItems, query}: CollectionListProps) => {
  ...
  const [itemsOffset, setItemsOffset] = useState(isMarketTab ? 0 : NUMBER_OF_ITEMS_PER_PAGE);

  const [marketItems, setMarketItems] = useState<OrdUtxo[]>(initialMarketItems);
  const [items, setItems] = useState<OrdUtxo[]>(initialCollectionItems);

  const [hasMoreMarket, setHasMoreMarket] = useState(initialMarketItems.length >= NUMBER_OF_ITEMS_PER_PAGE);
  const [hasMoreItems, setHasMoreItems] = useState(initialCollectionItems.length >= NUMBER_OF_ITEMS_PER_PAGE);
  ...
  const loadMore = async () => {
    if (isMarketTab) {
      const newItems = await fetchCollectionMarket(query, marketOffset) ?? [];
      ...
      const newItems = await fetchCollectionItems(query, itemsOffset) ?? [];
  ...
  return (
    ...
    : <div className='mx-auto my-4 text-center'>{`No ${tab}${isMarketTab ?  ' items' : ''} for this collection`}</div>
  )
}
