...
export const CollectionList = ({ collectionId }: CollectionListProps) => {
  ...
  const hasMore = useMemo(() => isMarketTab ? hasMoreMarket : hasMoreItems, [
    isMarketTab,
    hasMoreMarket,
    hasMoreItems,
  ]);

  const itemsToDisplay = useMemo(() => isMarketTab ? marketItems : items, [
    isMarketTab,
    marketItems,
    items,
  ]);
  ...
  const setLoadingStatus = useCallback((tab: string | Tab, status: FetchStatus) => {
  ...
  }, [collectionId, isMarketTab, itemsOffset, marketOffset, loading]);
  ...
  useEffect(() => {
    ...
    if (itemsOffset === 0 && !isMarketTab && loading.get(Tab.Items) === FetchStatus.Idle) {
      fire();
    }
    if (marketOffset === 0 && isMarketTab && loading.get(Tab.Market) === FetchStatus.Idle) {
      fire();
    }
  ...
  return (
    <section className="w-full flex flex-col items-center justify-center">
      ...
      {itemsToDisplay.length === 0 && loading.get(tab) === FetchStatus.Success && (
        <div className="mx-auto my-4">{`No ${tab}${isMarketTab ? " items" : ""
          } for this collection`}</div>
      )}
      {loading.get(tab) === FetchStatus.Success && itemsToDisplay.length > 0 && hasMore && (
