...
const SearchPage: React.FC<SearchPageProps> = (props) => {
	...
	const {
		data: bsv21Results,
		isLoading: isLoadingBsv21,
		isError: isErrorBsv21,
	} = useQuery({
	...
	const Listings = () => {
		switch (selectedAssetType) {
			case AssetType.Ordinals:
				return <OrdinalListings term={termFromPath} mode={OrdViewMode.List} />;
	...
	return (
		<div className="w-full max-w-5xl mx-auto">
			...
			{bsv21Results && bsv21Results.length > 0 && (
				<div className="text-[#555] font-semibold text-lg mb-2">BSV21</div>
			)}
			{bsv21Results && bsv21Results.length > 0 && (
				<div className="w-full text-foreground grid grid-cols-8 mb-4 gap-2">
						...
						>
							<Link href={`/market/bsv21/${match.id}`}>
								<div className="flex items-center">
									...
									<div className="inline-flex">{match.tick}</div>
								</div>
							</Link>
						</Button>
					))}
			...
			{bsv20Results && bsv20Results.length > 0 && (
				<div className="text-[#555] font-semibold text-lg mb-2">Ordinals</div>
			)}
