				...
				<div className="p-2 md:p-6 h-full w-full text-white bg-transparent bottom-0 left-0 bg-gradient-to-t from-black from-60% to-transparent block">
					{loading && (
						<div className="text-center h-full flex items-center justify-center">
							<Loader2Icon className="animate-spin w-4 h-4 m-auto" />
						</div>
					)}

					<div
						className={`${loading ? "opacity-0" : "opacity-100"} px-2 transition-opacity duration-500`}
					>
								...
								<div className="flex justify-between">
									...
									<div>
										{Object.keys(pendingTx.value?.metadata).map((k) => {
											const v = pendingTx.value?.metadata?.[k];
											return (
					...
					<Separator
						className={`${loading ? "opacity-0" : "opacity-100"} transition-opacity duration-750`}
					/>
					<div
						className={`${loading ? "opacity-0" : "opacity-100"} transition-opacity duration-750 mx-auto text-center text-teal-700 mb-2`}
					>
						{pendingTx.value?.txid}
					</div>

					<div
						className={`${loading ? "opacity-0" : "opacity-100"} flex gap-2 items-center justify-between mb-8 transition-opacity duration-1000`}
					>
						<div
					...
					<div
						className={`${loading ? "opacity-0" : "opacity-100"} transition-opacity duration-3000 items-center justify-center gap-2`}
					>
						<Button
							type="button"
							...
							onClick={async () => {
								if (pendingTx.value) {
									...
									toast.error(
										"No pending transaction to broadcast",
										toastProps,
									);
								}
							}}
