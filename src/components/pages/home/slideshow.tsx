...
const SlideShow = ({ artifacts, className }: Props) => {
  ...
  return (
    <div className={`mx-auto max-w-5xl my-8 stack ${className ? className : ''}`}>
      {sorted?.map((artifact, idx, all) => (
        <Artifact
          ...
          classNames={{
            wrapper: "w-[90vw] md:w-full md:min-w-96 md:max-w-[450px] flex items-center justify-center",
            media: "w-full rounded",
          }}
          size={600}
          to={`/outpoint/${artifact?.outpoint}`}
          priority={true}
