...
function getCollectionId(item: OrdUtxo): string {
	return (
		item.origin?.data?.map?.subTypeData?.collectionName ||
...
async function refreshFeedPool() {
	...
	(async () => {
		try {
			...
			const promises = [
				fetchWithTimeout(
					`${API_HOST}/api/market?limit=40&offset=0&type=image`,
					3000,
				),
				fetchWithTimeout(
					`${API_HOST}/api/market?limit=40&offset=40&type=image`,
					3000,
				),
				fetchWithTimeout(
					`${API_HOST}/api/market?limit=10&offset=0&type=video`,
					3000,
				),
			];
...
async function backgroundExpand() {
	try {
		...
		const promises = offsets.map((offset) =>
			fetchWithTimeout(
				`${API_HOST}/api/market?limit=20&offset=${offset}&type=image`,
				3000,
			),
		);
		...
		console.log(
			`Expanded pool by ${withTimestamps.length} items. Total: ${feedPool.length}`,
		);
	} catch (error) {
...
export async function GET(request: NextRequest) {
	...
	const items = feedPool
		.slice(start, end)
		.map(({ _cachedAt, ...item }) => item);
