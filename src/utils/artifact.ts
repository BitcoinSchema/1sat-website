...
console.log({txo})
  switch (type) {
    ...
    case ArtifactType.Image:
      return latest ? txo.data?.map?.name || txo.data?.map?.subTypeData?.name || txo.data?.map?.app || "Unknown Name" : txo.origin?.data?.map?.name || txo.origin?.data?.map?.subTypeData?.name || txo.origin?.data?.map?.app || "Unknown Name";
