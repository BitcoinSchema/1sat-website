...
export const fillContentType = async (artifact: OrdUtxo): Promise<OrdUtxo> => {
	...
	try {
		...
		if (response.status !== 404) {
			...
			const f = {
				hash: Utils.toHex(
					Hash.sha256(Buffer.from(buff).toString("hex"), "hex"),
				),
				size: blob.size,
				type: blob.type,
			};
...
export const displayName = (
	...
	switch (type) {
		...
		case ArtifactType.HTML: {
			const nameFromMeta =
				txo.origin?.data?.map?.name || txo.origin?.data?.map?.subTypeData?.name;
			if (nameFromMeta) {
				return nameFromMeta;
			}
			const html = !latest
		...
		case ArtifactType.BSV20:
			return latest
				? txo.data?.bsv20?.tick || txo?.data?.insc?.json?.sym
				: txo.origin?.data?.bsv20?.tick || txo.origin?.data?.insc?.json?.sym;
		case ArtifactType.LRC20: {
			const data = latest
				? (txo.data?.insc as Inscription)
				: (txo.origin?.data?.insc as Inscription);
			return `${data.json.p} ${data.json.op} ${data.json.amt}`;
		}
		case ArtifactType.OPNS: {
			const data = latest
				? (txo.data?.opns as OpNSData)
				: (txo.origin?.data?.opns as OpNSData);
