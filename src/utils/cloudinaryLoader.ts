...
export const cloudinaryLoader = ({ src, width, quality }: { src: string; width: number; quality?: string }) => {
  const params = ['f_auto', 'c_limit', `w_${width}`, `q_${quality || 'auto'}`];
  return `https://res.cloudinary.com/${cloudName}/image/upload/${params.join(',')}/${normalizeSrc(src)}`;
}

export const cloudinaryFetchLoader = ({ src, width, quality }: { src: string; width: number; quality?: string }) => {
  const params = ['f_auto', 'c_limit', `w_${width}`, `q_${quality || 'auto'}`];
