 ...
 */
export function getImageUrl(source: string, options: ImageOptions = {}): string {
  const { width, height, fit = "cover", background, format = "auto", quality } = options;
 ...
 */
export function getOrdinalThumbnail(outpoint: string, size: number = 300): string {
 ...
 */
export function getCoverImageUrl(source: string, width: number, height?: number): string {
  return getImageUrl(source, {
    width,
    height: height || width,
    fit: "cover",
