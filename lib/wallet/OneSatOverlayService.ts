...
export class OneSatOverlayService {
  ...
  async getAllOrdinals(): Promise<OneSatTxo[]> {
    ...
    const pureOrdinals = allOutputs.filter(txo => !tokenOutpoints.has(txo.outpoint));

    console.log(`[OneSatOverlay] Outputs: ${allOutputs.length} total, ${tokens.length} tokens (application/bsv-20), ${pureOrdinals.length} pure ordinals`);
  ...
  async getAllTokens(): Promise<{ bsv20: OneSatTxo[]; bsv21: OneSatTxo[] }> {
    ...
    console.log(`[OneSatOverlay] Tokens: ${allTokens.length} total (application/bsv-20 inscription type)`);
  ...
  async getAllOrdinalsAndTokens(): Promise<{
    ...
    // Filter to tokens (inscription type = application/bsv-20)
    const tokens = allOutputs
      .filter(txo => this.isToken(txo))
      .map(parseTxo);
    ...
    console.log(`[OneSatOverlay] Fetched ${allOutputs.length} outputs: ${ordinals.length} ordinals (NFTs), ${tokens.length} tokens (BSV20/BSV21)`);
   ...
   */
  async syncToWallet(wallet: any): Promise<{ balance: number; ordinalsCount: number; tokensCount: number }> {
    ...
    console.log(`[OneSatOverlay] Synced: ${balance?.total || 0} sats, ${ordinals.length} ordinals, ${tokens.length} tokens`);
