...
function generateMockActivity(page: number): ActivityItem[] {
  ...
  return Array.from({ length: 10 }, (_, i) => {
    ...
    return {
      ...
      data: {
        amount: type === "bsv20-transfer" ? Math.floor(Math.random() * 10000) : undefined,
        ticker: type === "bsv20-transfer" ? tickers[Math.floor(Math.random() * tickers.length)] : undefined,
...
export async function fetchActivity({
  pageParam = 0,
}: { pageParam?: number }): Promise<ActivityPage> {
